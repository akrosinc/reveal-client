...
public class DownloadedOfflineMapsFragment extends BaseOfflineMapsFragment implements DownloadedOfflineMapsContract.View, View.OnClickListener, OfflineRegion.OfflineRegionObserver {
    ...
    public void downloadMap(@NotNull List<OfflineMapModel> offlineMapsToDelete) {
        for (OfflineMapModel model : offlineMapsToDelete) {
            if (model.getOfflineRegion() == null) continue;
    ...
    public void deleteDownloadedOfflineMaps() {
        ...
        for (OfflineMapModel offlineMapModel : offlineMapsTodelete) {
            if (offlineMapModel.getOfflineRegion() != null) {
                // Native Mapbox deletion replacing Kujaku's OfflineServiceHelper
                offlineMapModel.getOfflineRegion().delete(new OfflineRegion.OfflineRegionDeleteCallback() {
                    @Override
                    public void onDelete() {
                        mapDeletedSuccessfully(offlineMapModel.getDownloadAreaId());
                    }

                    @Override
                    public void onError(String error) {
                        displayError(R.id.delete_map, error);
                    }
                });
            }
        }
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.offline_map_checkbox) {
    ...
    public void onError(OfflineRegionError error) {
        Timber.e("Offline region error: reason=%s, message=%s", error.getReason(), error.getMessage());
    }
