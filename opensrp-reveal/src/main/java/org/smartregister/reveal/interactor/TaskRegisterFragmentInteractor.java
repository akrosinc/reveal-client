...
public class TaskRegisterFragmentInteractor extends BaseInteractor
    implements TaskRegisterFragmentContract.Interactor {
  ...
  public TaskRegisterFragmentInteractor(
      TaskRegisterFragmentContract.Presenter presenter, Float locationBuffer) {
    super(presenter);
    ...
    interactorUtils =
        new InteractorUtils(
            RevealApplication.getInstance().getTaskRepository(),
  ...
  private String mainSelect(String mainCondition) {
    ...
    queryBuilder.customJoin(
        String.format(
            " JOIN %s ON %s.%s = %s.%s ", STRUCTURES_TABLE, tableName, FOR, STRUCTURES_TABLE, ID_));
    if (getBuildCountry() != Country.NAMIBIA) {
      ...
      queryBuilder.customJoin(
          String.format(
              " LEFT JOIN %s ON %s.%s = %s.%s AND %s.%s = '%s'",
              SPRAYED_STRUCTURES,
              tableName,
              FOR,
              SPRAYED_STRUCTURES,
  ...
  private String nonRegisteredStructureTasksSelect(String mainCondition) {
    ...
    queryBuilder.addCondition(String.format(" AND %s.%s IS NULL", FAMILY, STRUCTURE_ID));
    return queryBuilder.addCondition(
        String.format(" AND %s.%s != '%s'", tableName, CODE, BEDNET_DISTRIBUTION));
  }
  ...
  private String groupedRegisteredStructureTasksSelect(String mainCondition) {
    ...
    String[] columns =
        ArrayUtils.add(
            ...
            String.format(
                "%s.%s||' '||%s.%s as %s ",
                FAMILY_MEMBER, FIRST_NAME, FAMILY_MEMBER, LAST_NAME, FAMILY_MEMBER_NAMES));
    structureTasksQueryBuilder.selectInitiateMainTable(tableName, columns, ID_);
    structureTasksQueryBuilder.customJoin(
        String.format(
            " JOIN %s ON %s.%s = %s.%s ",
            STRUCTURES_TABLE, tableName, STRUCTURE_ID, STRUCTURES_TABLE, ID_));
    structureTasksQueryBuilder.customJoin(
        String.format(
            " JOIN %s ON %s.%s = %s.%s AND %s.%s IS NULL COLLATE NOCASE",
    ...
    return String.format(
            " SELECT %s.* , SUM(CASE WHEN status='%s' THEN 1 ELSE 0 END ) AS %s , COUNT(_id ) AS %s, "
  ...
  private String indexCaseSelect() {
    return String.format(
        "SELECT * FROM %s WHERE %s = ? AND %s = ? AND %s NOT IN (%s) AND %s = ? ",
        TASK_TABLE,
        GROUPID,
        PLAN_ID,
        STATUS,
  ...
  public void findTasks(
      Pair<String, String[]> mainCondition,
      Location lastLocation,
      Location operationalAreaCenter,
      String houseLabel) {
    if (mainCondition == null
        || mainCondition.second == null
        || mainCondition.second.length < 3
        || mainCondition.second[0] == null) {
        ...
        .execute(
            () -> {
              ...
              if (Utils.isFocusInvestigationOrMDA()) { // perform task grouping
                ...
                tasks.addAll(
                    queryTaskDetails(
                        groupedRegisteredStructureTasksSelect(mainCondition.first),
                        mainCondition.second,
                        lastLocation,
                        operationalAreaCenter,
                        houseLabel,
                        true));

                tasks.addAll(
                    queryTaskDetails(
                        nonRegisteredStructureTasksSelect(mainCondition.first),
                        mainCondition.second,
                        lastLocation,
                        operationalAreaCenter,
                        houseLabel,
                ...
                tasks.addAll(
                    queryTaskDetails(
                        mainSelect(mainCondition.first),
                        mainCondition.second,
                        lastLocation,
                        operationalAreaCenter,
                        houseLabel,
              ...
              if (PreferencesUtil.getInstance().isGdrsPlan() != null
                  && PreferencesUtil.getInstance().isGdrsPlan().equals("TRUE")) {
                ...
                tasks.sort(
                    new Comparator<TaskDetails>() {
                      ...
                      public int compare(TaskDetails o1, TaskDetails o2) {
                        Timber.tag("syncCreatedTaskToServer")
                            .i("sort %s %s", o1.getHouseHoldId(), o2.getHouseHoldId());
  ...
  private List<TaskDetails> queryTaskDetails(
      String query,
      String[] params,
      Location lastLocation,
      Location operationalAreaCenter,
      String houseLabel,
      boolean groupedTasks) {
    List<TaskDetails> tasks = new ArrayList<>();
    Cursor cursor = null;
    try {
  ...
  private TaskDetails readTaskDetails(
      Cursor cursor,
      Location lastLocation,
      Location operationalAreaCenter,
      String houseLabel,
      boolean isGroupedTasks) {
    ...
    if (isGroupedTasks) {
      ...
      task.setGroupedTaskCodeStatus(
          cursor.getString(cursor.getColumnIndex(GROUPED_STRUCTURE_TASK_CODE_AND_STATUS)));
    ...
    if (CASE_CONFIRMATION.equals(task.getTaskCode())) {
      ...
      if (task.getFamilyName() != null) task.setFamilyName(task.getFamilyName() + " " + houseLabel);
  ...
  private void calculateDistance(
      TaskDetails task, Location location, Location lastLocation, Location operationalAreaCenter) {
            ...
            () -> {
              ...
              for (TaskDetails taskDetails : tasks) {
                if (!BCC.equals(taskDetails.getTaskCode())
                    && !CASE_CONFIRMATION.equals(taskDetails.getTaskCode())) {
  ...
  public void getHdssDetailsForTasks(
      List<TaskDetails> tasks, Location location, int structuresWithinBuffer) {
    if (tasks == null) return;

    for (TaskDetails taskDetails : tasks) {
      if (List.of(
              Constants.Action.RCD,
              Constants.Action.INDEX_CASE,
              Constants.Action.SECONDARY_INDEX_CASE)
          .contains(taskDetails.getTaskCode())) {

        List<HdssCompoundHousehold> compoundAndHouseholdByStructureId =
            hdssRepository.getCompoundAndHouseholdListByStructureId(taskDetails.getTaskEntity());


        if (compoundAndHouseholdByStructureId != null
            && !compoundAndHouseholdByStructureId.isEmpty()) {
          taskDetails.setCompoundId(compoundAndHouseholdByStructureId.get(0).getCompoundId());
          taskDetails.setHouseHoldId(compoundAndHouseholdByStructureId.stream().map(
              HdssCompoundHousehold::getHouseholdId).collect(
              Collectors.joining("/")));
        }
      }
    }
    tasks.sort(
        new Comparator<TaskDetails>() {
          @Override
          public int compare(TaskDetails o1, TaskDetails o2) {
            Timber.tag("syncCreatedTaskToServer async")
                .i("sort %s %s", o1.getHouseHoldId(), o2.getHouseHoldId());
            return o1.getHouseHoldId().compareTo(o2.getHouseHoldId());
          }
        });
    ...
    appExecutors
        ...
        .execute(
            () -> {
              ...
              else structure = structureRepository.getLocationById(taskDetails.getStructureId());
              appExecutors
                  .mainThread()
                  .execute(
                      () -> {
