...
public class Utils {
  ...
  public static void setTextViewText(
      @NonNull TextView textView, @NonNull @StringRes Integer labelResource, String value) {
    SpannableStringBuilder builder = new SpannableStringBuilder();
    ForegroundColorSpan blackSpan =
        new ForegroundColorSpan(textView.getResources().getColor(R.color.text_black));
    builder.append(textView.getResources().getString(labelResource)).append(" ");
    int start = builder.length();
    builder
        .append(value)
        .setSpan(blackSpan, start, builder.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
  ...
  public static Float getLocationBuffer(boolean isTargetLevelStructure) {
    return isTargetLevelStructure
        ? Float.valueOf(
            getGlobalConfig(
                CONFIGURATION.LOCATION_BUFFER_RADIUS_IN_METRES,
                BuildConfig.MY_LOCATION_BUFFER + ""))
        : Float.valueOf(
            getGlobalConfig(
                CONFIGURATION.LOCATION_BUFFER_RADIUS_IN_METRES_FOR_LITE,
                BuildConfig.MY_LOCATION_BUFFER + ""));
  }

  public static long getSyncInterval() {
    return Long.parseLong(
        getGlobalConfig(CONFIGURATION.SYNC_INTERVAL_IN_MINUTES, getDefaultSyncInterval()));
  }
  ...
  public static Float getPixelsPerDPI(Resources resources) {
    return TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1, resources.getDisplayMetrics());
  }
  ...
  public static int getInterventionLabel() {
    ...
    if (interventionType.equals(FI) || interventionType.equals(DYNAMIC_FI))
      ...
      return R.string.mda;
    else if (interventionType.equals(MDA) || interventionType.equals(MDA)) return R.string.mda;
    else if (interventionType.equals(MDA_LITE)) return R.string.mda_lite;
    else if (SURVEY.equals(interventionType)) {
      return R.string.survey_coverage;
    } else return R.string.irs;
  }
  ...
  /**
   * Uses the server setting "draw_operational_area_boundary_and_label" to determine whether to draw
   * the operational area boundary If this variable is not available on the server the
   * DEFAULT_DRAW_OPERATIONAL_AREA_BOUNDARY_AND_LABEL value from the constants file is used
   *
  ...
  public static Boolean getDrawOperationalAreaBoundaryAndLabel() {
    return Boolean.valueOf(
        getGlobalConfig(
            CONFIGURATION.DRAW_OPERATIONAL_AREA_BOUNDARY_AND_LABEL,
            CONFIGURATION.DEFAULT_DRAW_OPERATIONAL_AREA_BOUNDARY_AND_LABEL.toString()));
  }

  /**
   * Uses the server setting "validate_far_structures" to determine whether to Validate Far
   * Structures If this variable is not available on the server the value is retrieved from
   * BuildConfig.VALIDATE_FAR_STRUCTURES
   *
  ...
  public static Boolean validateFarStructures() {
    return Boolean.valueOf(
        getGlobalConfig(
            CONFIGURATION.VALIDATE_FAR_STRUCTURES, BuildConfig.VALIDATE_FAR_STRUCTURES + ""));
  }

  /**
   * Uses the server setting "resolve_location_timeout_in_seconds" to determine the Resolve Location
   * Timeout In Seconds value If this variable is not available on the server the value is retrieved
   * from BuildConfig.RESOLVE_LOCATION_TIMEOUT_IN_SECONDS
   *
  ...
  public static int getResolveLocationTimeoutInSeconds() {
    return Integer.valueOf(
        getGlobalConfig(
            CONFIGURATION.RESOLVE_LOCATION_TIMEOUT_IN_SECONDS,
            BuildConfig.RESOLVE_LOCATION_TIMEOUT_IN_SECONDS + ""));
  }

  /**
   * Uses the server setting "admin_password_not_near_structures" to determine the Admin Password
   * required to perform any edits when not near a structure If this variable is not available on
   * the server the value is retrieved from BuildConfig.ADMIN_PASSWORD_NOT_NEAR_STRUCTURES
   *
   * @return AdminPassword
   */
  public static String getAdminPasswordNotNearStructures() {
    return getGlobalConfig(
        CONFIGURATION.ADMIN_PASSWORD_NOT_NEAR_STRUCTURES,
        BuildConfig.ADMIN_PASSWORD_NOT_NEAR_STRUCTURES);
  }

  /**
   * Creates a circle using a GeoJSON polygon. It's not strictly a circle but by increasing the
   * number of sides on the polygon you can get pretty close to one.
   *
   * <p>Adapted from
   ...
   * @param points - Since this is a GeoJSON polygon, we need to have a large number of sides so
   *     that it gets as close as possible to being a circle
   * @return
   * @throws Exception
   */
  public static Feature createCircleFeature(LatLng center, Float radius, Float points)
    ...
    double distanceX =
        radiusInKm
            / (KILOMETERS_PER_DEGREE_OF_LONGITUDE_AT_EQUITOR
                * Math.cos(center.getLatitude() * Math.PI / 180));
  ...
  public static boolean isResidentialStructure(String taskCode) {
    ...
    return !(MOSQUITO_COLLECTION.equals(taskCode)
        || LARVAL_DIPPING.equals(taskCode)
        || PAOT.equals(taskCode));
  }
   ...
   *
   * <p>If this variable is not available on the server the
   * DEFAULT_DRAW_OPERATIONAL_AREA_BOUNDARY_AND_LABEL value from the constants file is used
   *
  ...
  public static Boolean displayAddStructureOutOfBoundaryWarningDialog() {
    return Boolean.valueOf(
        getGlobalConfig(
            CONFIGURATION.DISPLAY_ADD_STRUCTURE_OUT_OF_BOUNDARY_WARNING_DIALOG,
            CONFIGURATION.DEFAULT_DISPLAY_ADD_STRUCTURE_OUT_OF_BOUNDARY_WARNING_DIALOG.toString()));
  }
  ...
  public static String getCurrentLocationId() {
    Location currentOperationalArea =
  ...
  public static void recreateEventAndClients(
      String query,
      String[] params,
      SQLiteDatabase db,
      FormTag formTag,
    ...
    try {
      ...
      Pair<List<Event>, List<Client>> events =
          util.createEventAndClients(db, tableName, query, params, eventType, entityType, formTag);
  ...
  public static boolean matchesSearchPhrase(String toSearch, String searchPhrase) {
    if (StringUtils.isBlank(toSearch)) return false;
    String wordsSpaceAndCommaRegex = "[\\w\\h,]*";
    return toSearch
        .toLowerCase()
        .matches(wordsSpaceAndCommaRegex + searchPhrase.toLowerCase() + wordsSpaceAndCommaRegex);
  }
  ...
  public static Set<String> getInterventionUnitCodes(Set<String> filterList) {
    ...
    if (filterList.contains(Constants.InterventionType.PERSON)
        || filterList.contains(Constants.InterventionType.FAMILY)) {
  ...
  /**
   * Uses the server setting "DISPLAY_DISTANCE_SCALE" to determine whether to display the distance
   * scale If this variable is not available on the server the value is retrieved from
   * BuildConfig.DISPLAY_DISTANCE_SCALE
   *
  ...
  public static Boolean displayDistanceScale() {
    return Boolean.valueOf(
        getGlobalConfig(
            CONFIGURATION.DISPLAY_DISTANCE_SCALE, BuildConfig.DISPLAY_DISTANCE_SCALE + ""));
  }

  /**
   * This method takes in a geometry object and returns a JSONArray representation of the
   * coordinates
   *
   * @param updatedGeometry The geometry of the updated feature
   * @param originalGeometry The geometry of the original feature used to determine whether it was a
   *     MultiPolygon or a Polygon
   * @return
   */
  public static JSONArray getCoordsFromGeometry(
      Geometry updatedGeometry, Geometry originalGeometry) {
    JSONObject editedGeometryJson;
    JSONArray updatedCoords = null;
    try {
      if (originalGeometry instanceof MultiPolygon) {
        MultiPolygon editedGeometryMultiPolygon =
            MultiPolygon.fromPolygon((Polygon) updatedGeometry);
  ...
  /**
   * This method takes in a view and a predicate. Displays the view if predicate is true. Hides the
   * view otherwise
   *
   ...
   * @return
   */
  public static LinkedHashMap<String, HashMap<String, String>> buildRepeatingGroup(
      @NonNull JSONObject jsonObject, List<Obs> obs) {
        ...
        if (values != null && !values.isEmpty() && fieldKey.contains("_")) {
          ...
          if (keysArrayList.contains(fieldKey)) {
            ...
            if (StringUtils.isNotBlank(fieldValue)) {
              String fieldKeyId =
                  valueField.getFormSubmissionField().substring(fieldKey.length() + 1);
   ...
   * @return
   */
  public static List<HashMap<String, String>> generateListMapOfRepeatingGrp(
  ...
  public static int getMaxZoomLevel() {
    return !isCurrentTargetLevelStructure()
        ? getSelectJurisdictionMaxSelectZoomLevel()
        : getMaxSelectZoomLevel();
  }
  ...
  public static boolean isRwandaMDALite() {
    return (!isCurrentTargetLevelStructure()
        && (Country.RWANDA.equals(getBuildCountry()) || getBuildCountry() == Country.RWANDA_EN));
  }
  ...
  public static boolean isMDALite() {
    return Country.KENYA.equals(getBuildCountry())
        || Country.RWANDA.equals(getBuildCountry())
        || getBuildCountry() == Country.RWANDA_EN && !isCurrentTargetLevelStructure();
  }

  public static Integer getMaxSelectZoomLevel() {
    return Integer.valueOf(
        getGlobalConfig(
            CONFIGURATION.MAX_SELECT_ZOOM_LEVEL, String.valueOf(MAX_SELECT_ZOOM_LEVEL)));
  }

  public static Integer getSelectJurisdictionMaxSelectZoomLevel() {
    return Integer.valueOf(
        getGlobalConfig(
            CONFIGURATION.SELECT_JURISDICTION_MAX_SELECT_ZOOM_LEVEL,
            String.valueOf(SELECT_JURISDICTION_MAX_SELECT_ZOOM_LEVEL)));
  }

  public static void logAdminPassRequiredEvent(
      android.location.Location location, boolean passwordEntered) {
    AllSharedPreferences sharedPreferences =
        new AllSharedPreferences(
            PreferenceManager.getDefaultSharedPreferences(
                RevealApplication.getInstance().getApplicationContext()));
  ...
  public static boolean buildCountryHasIndicators() {
    return getBuildCountry() == Country.ZAMBIA
        ...
        || getBuildCountry() == Country.GDRS
        || getBuildCountry() == Country.NIH;
  }
  ...
  public static String getSatelliteStyle(Context context) {
    return org.smartregister.reveal.util.Utils.isCurrentTargetLevelStructure()
        ? context.getString(R.string.reveal_satellite_style)
        : context.getString(R.string.reveal_select_jurisdiction_style);
  }
}
