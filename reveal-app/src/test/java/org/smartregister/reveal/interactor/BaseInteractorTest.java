...
import static com.revealprecision.reveal.util.Constants.JsonForm.CDD_SUPERVISOR_DAILY_SUMMARY_FORM;
import static com.revealprecision.reveal.util.Constants.JsonForm.LOCATION;
import static com.revealprecision.reveal.util.Constants.JsonForm.PAOT_STATUS;
import static com.revealprecision.reveal.util.Constants.JsonForm.TABLET_ACCOUNTABILITY_FORM;
import static com.revealprecision.reveal.util.Constants.Properties.APP_VERSION_NAME;
import static com.revealprecision.reveal.util.Constants.Properties.LOCATION_PARENT;
import static com.revealprecision.reveal.util.Constants.Properties.LOCATION_UUID;
import static com.revealprecision.reveal.util.Constants.Properties.PLAN_IDENTIFIER;
...
public class BaseInteractorTest extends BaseUnitTest {
    ...
    public void testSavePaotForm() throws JSONException {
        ...
        String form = AssetHandler.readFileFromAssetsFolder(com.revealprecision.reveal.util.Constants.JsonForm.PAOT_FORM, context);
        ...
        assertEquals(com.revealprecision.reveal.util.Constants.EventType.PAOT_EVENT, eventJSONObjectCaptor.getValue().getString("eventType"));
    ...
    public void testSaveRegisterStructureForm() throws JSONException {
        String form = AssetHandler.readFileFromAssetsFolder(com.revealprecision.reveal.util.Constants.JsonForm.ADD_STRUCTURE_FORM, context);
    ...
    public void testSaveMemberForm() throws JSONException {
        String form = AssetHandler.readFileFromAssetsFolder(com.revealprecision.reveal.util.Constants.JsonForm.BLOOD_SCREENING_FORM, context);
        JSONObject formObject = new JSONObject(form);
        ...
        details.put(TASK_IDENTIFIER, taskId);
        formObject.put(DETAILS, details);
        JsonFormUtils.getFieldJSONObject(JsonFormUtils.fields(formObject), "business_status").put(VALUE, com.revealprecision.reveal.util.Constants.BusinessStatus.COMPLETE);
        ...
        assertEquals(com.revealprecision.reveal.util.Constants.BusinessStatus.COMPLETE, obs.getJSONObject(2).getJSONArray(VALUES).get(0));
    }

    @Test
    public void testSaveCaseConfirmation() throws JSONException {
        String form = AssetHandler.readFileFromAssetsFolder(com.revealprecision.reveal.util.Constants.JsonForm.CASE_CONFIRMATION_FORM, context);
        String planIdentifier = UUID.randomUUID().toString();
        PreferencesUtil.getInstance().setCurrentPlanId(planIdentifier);
        JSONObject formObject = new JSONObject(form);
        String entityId = UUID.randomUUID().toString();
        ...
        assertEquals(com.revealprecision.reveal.util.Constants.EventType.CASE_CONFIRMATION_EVENT, eventJSONObjectCaptor.getValue().getString("eventType"));
    ...
    public void testEntityIdOnSaveTableAccountabilityForm() throws Exception {
        ...
        String searchQuery = String.format("select %s from  %s where %s = ? limit 1", ID_, STRUCTURES_TABLE, com.revealprecision.reveal.util.Constants.DatabaseKeys.NAME);
